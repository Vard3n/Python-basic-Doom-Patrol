...
class TestUserFunctions(unittest.TestCase):
    ...
    @patch('user_function.input')
    def test_update_user(self, input_mok):
        file = create_file()
        file.close()
        with open(Config.PATH_TO_USERS_FILE, 'w') as file:
            file.write(json.dumps(TEST_ALL_USERS_DATA))
        input_mok.return_value = TEST_ID_UPDATE
        update_user()
        ...
        file = open(Config.PATH_TO_USERS_FILE, 'r')
        data = json.loads(file.read())
        self.assertEqual(data[0]['first_name'], TEST_ID_UPDATE)
        self.assertEqual(data[0]['last_name'], TEST_ID_UPDATE)
        self.assertEqual(data[0]['Email'], TEST_ID_UPDATE)
        file.close()
        shutil.rmtree(TEST_DATABASE_DIRECTORY)
